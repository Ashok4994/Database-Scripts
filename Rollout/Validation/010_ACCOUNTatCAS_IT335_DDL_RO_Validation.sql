/*
  Script:      010_ACCOUNTatCAS_IT335_DDL_RO_Validation.sql
  Author:      Carlos Mendoza
  Date:        06/24/2019
  Purpose:     I-T335 DDL Rollout validation script for ACCOUNT schema.  
*/
...
spool  010_ACCOUNTatCAS_IT335_DDL_RO_Validation_&filename1..log
...
IF (V_COUNT = 1) THEN
	str_audit := str_audit || 'TABLE T_DIGITAL_FORM_SUBMISSION HAS BEEN CREATED!' || chr(13);		
ELSE
...
IF (V_COUNT = 1) THEN
	str_audit := str_audit || ' SEQUENCE SEQ_DIGITAL_FORM_SUBMISSION HAS BEEN CREATED! ' || chr(13);
ELSE
	str_err := str_err || ' VALIDATION FAILED CHECKING FOR SEQUENCE NAME SEQ_DIGITAL_FORM_SUBMISSION! ' || chr(13);
END IF;


--Validate table T_DIGITAL_FORM_SUBMISSION_HISTORY
SELECT COUNT(1) 
INTO v_count 
FROM all_tables u 
...
IF (V_COUNT = 1) THEN
	str_audit := str_audit || 'TABLE T_DIGITAL_FORM_SUBMISSION_HISTORY HAS BEEN CREATED!' || chr(13);		
ELSE
...
IF (V_COUNT = 1) THEN
	str_audit := str_audit || ' SEQUENCE SEQ_DIGITAL_FORM_SUBMISSION_HISTORY HAS BEEN CREATED! ' || chr(13);
ELSE
...
IF (V_COUNT = 1) THEN
	str_audit := str_audit || 'TABLE T_LU_DIGITAL_FORM_DESCRIPTION HAS BEEN CREATED!' || chr(13);		
ELSE
...
ELSE
  str_err := str_err || '- ' || str_schema || '.' || str_syn || v_ext ||
             ' package IS NOT in the database or invalid!' || chr(13);
END IF;
